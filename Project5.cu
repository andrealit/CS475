...
__global__ void MonteCarlo(IN float* dtxs, IN float* dtys, IN float* dtxvs, IN float* dsvs, IN float* dsths, IN float* dhalflens, OUT int* dhits)
{
	...
	float sthr = Radians(sthd); // theta in radians
	float svx = sv * cos(sthr); // x component of thrown snowball
	float svy = sv * sin(sthr); // y component of thrown snowball
...
main(int argc, char* argv[])
{
	...
	cudaMemcpy(dhalflens, dhalflens, NUMTRIALS * sizeof(float), cudaMemcpyHostToDevice);
	CudaCheckError();
